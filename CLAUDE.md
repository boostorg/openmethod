...
```
`detail::tuple` (defined in `preamble.hpp`) is a minimal head/tail tuple used instead of
`std::tuple` — which is very expensive to instantiate with MSVC — for the policy-state tuple, the
`use_classes` registrar tuple, and `method::override::impl`; elements are retrieved by type with
`detail::get`. The `initialize()`/`finalize()` *options* tuple deliberately remains `std::tuple`:
it is a documented policy-API signature.
